...
struct enc_handle
{
	...
	struct enc_buf EncodeBuf;
};

struct enc_SPSPPS encSPSPPS;
struct enc_handle *handle;
...
{
	handle = malloc(sizeof(struct enc_handle));
	if (!handle)
	{
...
void encode_force_Ipic(void)
{
	...
	OMX_ERRORTYPE ret = OMX_SetConfig(ILC_GET_HANDLE(handle->video_encode), OMX_IndexConfigBrcmVideoRequestIFrame, &requestIFrame);
	if (ret != OMX_ErrorNone)
	{
