...
static void yuv422_to_yuv420(uint8_t *inbuf, uint8_t *outbuf, int width, int height)
{
	int i = 0, j = 0, k = 0;
...
struct cvt_handle *convert_open(struct cvt_param param)
{
	...
	if (handle->params.inpixfmt != V4L2_PIX_FMT_YUYV || handle->params.outpixfmt != V4L2_PIX_FMT_YUV420)
	{
		printf("--- Only YUV422 to YUV420 converting is supported\n");
		goto err0;
	}

	if (handle->params.inwidth != handle->params.outwidth || handle->params.inheight != handle->params.outheight)
	{
		printf("--- Scaling is not supported\n");
		goto err0;
	}

	handle->src_buffersize = handle->params.inwidth * handle->params.inheight * 16 / 8;			// YUV422 image size
	handle->dst_buffersize = handle->params.outwidth * handle->params.outheight * 12 / 8;		// YUV420 image size
	...
	printf("+++ Convert Opened, src buf size=%d, dst buf size=%d\n", handle->src_buffersize, handle->dst_buffersize);
	return handle;
...
int convert_do(struct cvt_handle *handle, const void *inbuf, int isize, void **poutbuf, int *posize)
{
	...
	yuv422_to_yuv420((uint8_t *) inbuf, handle->dst_buffer, handle->params.inwidth, handle->params.inheight);
